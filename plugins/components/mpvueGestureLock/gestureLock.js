class GestureLock {
    ...
    constructor(containerWidth, cycleRadius) {
        ...
        this.windowWidth = wx.getSystemInfoSync().windowWidth; // 窗口大小(用于upx 和 px 转换)
        ...
        for (let i = 0; i < 3; i++) {
            for (let j = 0; j < 3; j++) {
                ...
                this.circleArray.push({
                    count: count,
                    x: this.upxTopx((cycleMargin + this.cycleRadius) * (j * 2 + 1)),
                    y: this.upxTopx((cycleMargin + this.cycleRadius) * (i * 2 + 1)),
                    radius: this.upxTopx(this.cycleRadius),
                    check: false,
                    style: {
                        left: (cycleMargin + this.cycleRadius) * (j * 2 + 1) - this.cycleRadius + 'upx',
                        top: (cycleMargin + this.cycleRadius) * (i * 2 + 1) - this.cycleRadius + 'upx',
                        width: this.cycleRadius * 2 + 'upx',
                    }
                });
            }
        }
    ...
    // 将 upx 转换成 PX
    upxTopx(upx) {
        return upx / 750 * this.windowWidth;
    }
}
